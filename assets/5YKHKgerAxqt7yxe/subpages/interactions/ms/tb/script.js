...
function createStepListItem(stepText, stepIndex) {
  ...
  // 'Delete' button next to each step
  const deleteBtn = document.createElement('button');
  deleteBtn.textContent = 'Delete';
  deleteBtn.setAttribute('aria-label', `Delete step ${stepIndex + 1}`);
...
function addStep() {
  ...
  if (currentTaskIndex !== null) {
    ...
    // If adding a brand new task, use a temporary array
...
function getModalSteps() {
  ...
  allLis.forEach(li => {
    ...
    if (span) {
      stepTexts.push(span.textContent);
    }
  });
  return stepTexts;
}
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  // Allow pressing 'Enter' key to add a step
  ...
  // Allow pressing 'Enter' key to save the task
